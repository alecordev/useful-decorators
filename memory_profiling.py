...
log_config = {
    ...
    'handlers': {
        ...
        'logfile': {
            ...
            'formatter': 'detailed',
            'filename': '{}.log'.format(datetime.datetime.utcnow().strftime('%Y-%m-%d')),
            'mode': 'w'
        },
        'timer_handler': {
            'level': 'INFO',
    ...
    'loggers': {
        ...
        'timer_logger': {
            'handlers': ['timer_handler', 'console'],
            'level': 'INFO',
        },
    },
}
...
# Initializing global
performance_df = pd.DataFrame(columns=['timestamp', 'callable_', 'memory_diff', 'total_memory', 'description'])
...
def timing(f):
    ...
    def wrap(*args, **kwargs):
        ...
        end_time = time.time()
        msg = 'Processing: {} With args: [{}, {}] Took: {:2.4f} seconds'.format(f.__name__, args, kwargs, end_time - start_time)
...
def memory(f):
    ...
    def wrap(*args, **kwargs):
        ...
        performance_df = performance_df.append(dict(timestamp=datetime.datetime.utcnow().isoformat(), 
...
def measure_memory(msg):
    ...
    try:
        ...
        performance_df = performance_df.append(dict(timestamp=datetime.datetime.utcnow().isoformat(), 
                                                    total_memory=memory,
                                                    callable_=callee,
                                                    description=msg), ignore_index=True)
